...
# Define endpoints (Binary + Continous w/o threshold)
endpoints.HA <- list(
  list(type = "binary", prob = 0.96),
  list(type = "continuous", params = list(mu = 36, sigma = 24), threshold = 0)
)

# Define continuous endpoints for the null hypothesis (H0)
endpoints.H0 <- list(
  list(type = "binary", prob = 0.95),
  list(type = "continuous", params = list(mu = 31, sigma = 24), threshold = 0)
)
