...
# Define endpoints (Exp. Survival + Count with fixed censoring time)
endpoints.HA <- list(
  list(type = "survival", dist = "Exponential", params = list(lambda = 0.16)),
  list(type = "count", params = list(lambda = 0.75))
)

# Define continuous endpoints for the null hypothesis (H0)
endpoints.H0 <- list(
  list(type = "survival", dist = "Exponential", params = list(lambda = 0.20)),
  list(type = "count", params = list(lambda = 1.1))
)
