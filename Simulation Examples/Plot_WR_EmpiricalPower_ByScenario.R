...
output_pdf <- "Simulation Examples/Plot.WR.EmpiricalPower.ByScenario.pdf"
output_png <- "Simulation Examples/Plot.WR.EmpiricalPower.ByScenario.png"
output_csv <- "Simulation Examples/Plot.WR.EmpiricalPower.ByScenario.csv"
...
for (line in lines) {
  ...
  if (length(parts) < 11L) next
  
  records[[length(records) + 1L]] <- data.frame(
    scenario = current_scenario,
    fixed_n = current_n,
    rho = as.numeric(parts[1]),
    wr_empirical_power_pct = as.numeric(parts[9]),
...
p <- ggplot2::ggplot(
  plot_data,
  ggplot2::aes(x = rho, y = wr_empirical_power_pct, color = scenario_plot, group = scenario_plot)
) +
